export const fetchPokemon = async (sortOrder) => {
	const res = await fetch(
		`https://pokedex-alchemy.herokuapp.com/api/pokedex?perPage=166&sort=id&direction=${sortOrder}`
	);
...
export const fetchFilteredPokemon = async (selectedType, sortOrder) => {
	const res = await fetch(
		`https://pokedex-alchemy.herokuapp.com/api/pokedex?type=${selectedType}&perPage=166&sort=id&direction=${sortOrder}`
	);
